...
def submit_annotation(annotation: Annotation):
    ...
    row = {
        ...
        # Q1 multi
        "cells_present__B": "B cells are present" in a["Which cell types are present in the ELS?"],
        "cells_present__T": "T cells are present" in a["Which cell types are present in the ELS?"],
        "cells_present__Ki67": "Proliferating cells (Ki67+) are present" in a["Which cell types are present in the ELS?"],
...
def update_consensus(image_id):
    ...
    with engine.connect() as conn:
        conn.execute(
            text("""
                insert into image_consensus (image_id, {cols})
                values (:image_id, {vals})
                on conflict (image_id)
                do update set {updates}, updated_at = now()
            """.format(
                cols=", ".join(consensus.keys()),
                vals=", ".join([f":{k}" for k in consensus.keys()]),
                updates=", ".join([f"{k}=:{k}" for k in consensus.keys()])
            )),
